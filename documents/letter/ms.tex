...
on the mass of the unseen companion. However, since the systems should be isotropically
distributed in the inclination angle, for a large enough, unbiased sample, the 
...
companion stars given kinematic measurements of the primary. We use a mixture of 
(two) Gaussians to model the true companion mass distribution, meant to represent 
populations of white dwarfs (WD) and neutron stars (NS). We run this model on mock data generated
both with and without the same functional form as the fitting model in order to test the
...
distribution over NS fraction has a significant tail up to $f_{\rm NS} \sim 0.12$.
