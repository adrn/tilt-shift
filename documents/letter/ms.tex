...
efforts may compute the NS probability for newly discovered low mass WDs given a measured 
WD mass and mass function.
\end{abstract}
...
where the product is over all each of the $j$ systems.  The marginal likelihood involves integrals over the latent variables $i$ and $M_2$,
\begin{align}
...
\begin{deluxetable}{ccccc}
	...
	\startdata
		\multicolumn{2}{c}{\multirow{2}{*}{Priors}} & $\mathcal{U}(0.2, 1)$ & $\left[\ln(100)\sigma\right]^{-1}$  & $\mathcal{U}(0, 1)$ \\
		\multicolumn{2}{c}{} & & $(0.02 < \sigma/\Msun < 2)$ & \\
		\cutinhead{Test Cases}
		\multirow{2}{*}{Test 1} & True & 0.7 & 0.2 & 0.0 \\
		 & MAP & {\bf 0.74} & {\bf 0.18} & {\bf 0.0} \\
		 \hline
		\multirow{2}{*}{Test 2} & True & 0.7 & 0.2 & 0.1 \\
		 & MAP & {\bf 0.76} & {\bf 0.17} & {\bf 0.12} \\
		 \hline
		\multirow{2}{*}{Test 3} & True & \nodata & \nodata & 0.1 \\
		 & MAP & {\bf 0.80} & {\bf 0.39} & {\bf 0.12} \\
		 \cutinhead{ELM Sample}
		 & MAP & 0.71 & 0.26 & {\bf 0.0?}
	\enddata
...
\caption{The posteriors for our model applied to the ELM WDs. The first panel shows the MAP $M_2$ distribution (solid black) and random samples from the posterior (grey lines). The second panel shows the posterior distribution of $\mu$ and $\sigma$; The maximum a posteriori Gaussian model has $\mu_{\rm WD}= {\bf 0.71}~\Msun$ and $\sigma_{\rm WD}= {\bf 0.26}~\Msun$. The third panel the posterior distribution of $\mu$ and $f_{\rm NS}$. The last panel lists individual LMWDs in the ELM sample and shows each of their corresponding NS probabilities. The systems are ordered by increasing $\mf$. The three systems with $P_{\rm NS}=$0\% are the three eclipsing systems.}
\label{fig:ELM_post}
\end{center}
\end{figure*}
...
The results of using our model on the ELM sample are shown in Figure \ref{fig:ELM_post}. The maximum a posteriori model has parameters $\mu_{\rm WD} = {\bf 0.71} \Msun$, $\sigma_{\rm WD} = {\bf 0.26}~\Msun$, and $f_{\rm NS} = 0\%$. The marginal posterior over $\mu_{\rm WD}$ and $\sigma_{\rm WD}$ has a tail towards larger variances, which could indicate that the true WD distribution is not exactly Gaussian. However, it is interesting that the best fit Gaussian for the companions to the ELM WD sample is similar to that of the population of single DA WDs in SDSS with a mean of 0.6 $\Msun$ \citep{kleinman13}. Our distribution is significantly wider ({\bf $\sigma \approx 0.26~\Msun$} compared with $\sigma \approx 0.1~\Msun$), possibly due to mass transfer affecting the mass distribution of the unseen primary WDs as well.
...
%Since our results are still consistent with a 0\% NS fraction, it is not guaranteed that any of the LMWDs in the ELM WD sample have NS companions. However the fourth panel in Figure \ref{fig:ELM_post} indicates which ELM WDs have the highest probability of hosting NS companions. Here, we show the distributions of $P_{\rm NS}$ for each system, ordered by $\mf$. This panel shows that $P_{\rm NS}$ only starts to significantly deviate from zero for {\bf $\mf\approx0.6 \Msun$} Although a few systems have high probabilities, the distribution is strongly clustered near 0\%.
