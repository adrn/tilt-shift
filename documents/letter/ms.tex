...
We construct a statistical model to derive constraints on a parametric model --- parametrized by the vector $\bs{\theta}$ --- for the distribution of ELM WD companion masses, $M_2$.  For each system, we assume we are given $K$, $T$, and $M_1$ and therefore have a measurement of the mass function, $m_f$ (Eq.~\ref{eq:massfunc}). In this work, we neglect the uncertainties in the catalog parameters ($K$, $T$, $M_1$ and therefore $m_f$) because the relative uncertainties in these quantities are small, $\sigma_x / x \sim XXX$. We would therefore like to find posterior constraints on the parameters, $\boldsymbol{theta}$, given the ``observed'' mass functions, $m_f$. Using Bayes' rule,
\begin{equation}
    p(\bs{\theta} \given m_f, M_1) = \frac{1}{\mathcal{Z}}~p(m_f \given \bs{\theta})~p(\bs{\theta}).
\end{equation}
The likelihood, $p(m_f \given \bs{\theta})$, involves integrals over the unobserved inclination angle, $i$, and companion mass, $M_2$
\begin{equation}
