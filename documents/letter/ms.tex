...
\end{align}
where $\mathcal{N}$ is the (truncated, but properly normalized) normal distribution with mean $\mu$ and variance $\sigma^2$ and the distributions are limited to the ranges specified. In this work, we fix the mean and variance of the neutron star distribution to
\begin{align}
	\mu_{\rm NS} &= 1.4~\Msun\\
...
\begin{figure*}[h!]
\begin{center}
\includegraphics[angle=90,width=0.95\textwidth]{ELM_post.eps}
\caption{Replace this text with your caption}
\label{fig:ELM_post}
\end{center}
\end{figure*}
